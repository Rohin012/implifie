    ...
    <div class="flex space-x-5 chat-height overflow-hidden relative rtl:space-x-reverse">
      <div class="flex-none min-w-[260px]" v-if="window.width > 1024">
        <Card bodyClass=" relative p-0 h-full overflow-hidden ">
          <div>
            <div class="border-b border-slate-200 search px-3 mx-6 mt-1 flex items-center space-x-3 rtl:space-x-reverse">
              <div class="flex-none text-base text-slate-900 dark:text-slate-400">
                <Icon icon="bytesize:search" class="text-sm" />
              </div>
              <input v-model="this.searchContact" placeholder="Search Contacts ..."
                class="w-full flex-1 block bg-transparent placeholder:font-normal placeholder:text-sm placeholder:text-slate-400 py-2 focus:ring-0 focus:outline-none dark:text-slate-200 dark:placeholder:text-slate-400" />
            </div>
          </div>
      ...
      <Transition name="overlay-fade">
        <div v-if="window.width <= 1024 && this.mobileChatSidebar"
